...
def recognize_plate(img, coords):
    ...
    img_3d = cv2.drawContours(img, contours, -1, (0,128,0), 3)
    #cv2.imshow("Show",img_3d)
    #cv2.waitKey()
    ...
    for cnt in sorted_contours:
       ...
       # if height / float(h) > 10: continue # original = 6

        ratio = h / float(w)
        # if height to width ratio is less than 1.5 skip
        if ratio < 0.5: continue  # original = 1.5

        # if width is not wide enough relative to total width then skip
        if width / float(w) > 20: continue  # original = 15
        ...
        if class_name not in allowed_classes:
            ...
            if read_plate:
                ...
                if plate_number != None:
                    database = ['KL55R2473','FMA6383','STA5131E','KS67AEA','KA03MR3892']
                    flag = 1
                    for i in database:
                        if(i==plate_number):
                            print('Vehicle with plate ' + i + ' is authorized')
                            flag = 0
                    if(flag):
                        print('Vehicle with plate ' + i + ' is not authorized')
